...
class EncryptionKeySpec:
    ...
    def it_decrypts_itself_when_correct_password_given(self):
        (result, decrypted_key) = self.encryption_key.decrypt(
            "masterpassword123")
        decrypted_key = b64encode(decrypted_key).decode('utf8')

        eq_(result, True)
        eq_(decrypted_key, self.decrypted_key)
